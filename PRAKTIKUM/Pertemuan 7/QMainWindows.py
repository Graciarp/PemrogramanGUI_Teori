...
class MainForm(QMainWindow):
     ...
     def setupUi(self):
          ...
          editCutAction = QAction(QIcon('icons/Cut.png'),'C&ut', self)
     ...
     def confirmation(self):
          if self.textEdit.document().isModified():
               response = QMessageBox.question(self,'Konfirmasi','Teks telah dimodifikasi. Simpan?')
     ...
     def fileSaveAsActionTriggered(self):
          fileName = QFileDialog.getSaveFileName(self,'Simpan file', os.curdir,'File Teks (*.txt)','*.txt')
          if not fileName[0]: return
          self.currentFileName = fileName[0]
          self.setWindowTitle(PROGRAM_NAME + ' - ' +
          self.currentFileName)
